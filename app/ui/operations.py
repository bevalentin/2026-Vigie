...
from app.models.domain import Operation, Lot, BankAccount, Owner, OperationType, OperationCategory, Allocation, UserRole
...
def operations_page():
     ...
     with ui.dialog() as dialog, ui.card().classes('w-full max-w-2xl'):
        ...
        with ui.grid(columns=2).classes('w-full gap-4'):
             ...
             cat_select = ui.select([c.value for c in OperationCategory], label='Catégorie', value=OperationCategory.AUTRE.value)
             
             label_input = ui.input('Libellé').classes('col-span-2')
             
             # Distribution Recipient (only for REVERSEMENT)
             recipient_select = ui.select(owners_map, label='Propriétaire Bénéficiaire (si hors lot)', clearable=True).classes('col-span-2')
             recipient_select.bind_visibility_from(cat_select, 'value', backward=lambda v: v == OperationCategory.REVERSEMENT.value)
        ...
        def save():
             try:
                 ...
                 is_reversement = cat_select.value == OperationCategory.REVERSEMENT.value
                 ...
                 with next(get_session()) as session:
                     if op_id_ref['value']:
                         ...
                         op.category = OperationCategory(cat_select.value)
                         ...
                         op = Operation(
                             ...
                             category=OperationCategory(cat_select.value),
     ...
     with ui.dialog() as transfer_dialog, ui.card().classes('w-full max-w-2xl'):
        ...
        # Load helpers (re-load to ensure fresh data)
        # We can reuse maps if they are reactive or refreshed. 
        # For simplicity, we assume pages reload or we use the main page maps if available?
        # The maps inside 'operations_page' scope are effectively static after first load unless refreshed.
        # But for now let's reuse 'lots_map' and 'accounts_map' from outer scope which were loaded at page start.
        # Ideally we should reload them in a 'open_transfer' function.
        ...
        # t_lot removed for transfers (pure treasury movement)
        ...
        def execute_transfer():
             try:
                 ...
                 # if not t_lot.value: -> No longer needed

                 from app.services.accounting import create_transfer # Import logic
                 
                 with next(get_session()) as session:
                     # lot_id is None for treasury transfers
     ...
     def content():
        ...
        def open_create():
            ...
            cat_select.value = OperationCategory.AUTRE.value
            label_input.value = ''
